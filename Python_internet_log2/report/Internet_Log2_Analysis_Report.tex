...
\bottomrule
\end{tabular}
\caption{Categorical Variable Cardinality Analysis}
\end{table}
...
\begin{itemize}
    \item Bytes, Bytes Sent, Bytes Received: VIF = ∞ (perfect multicollinearity)
    \item Packets, pkts\_sent, pkts\_received: VIF = ∞ (perfect multicollinearity)
...
\bottomrule
\end{tabular}
\caption{SVM Cross-Validation Performance by C Parameter}
\end{table}
...
\bottomrule
\end{tabular}
\caption{SGD 5-Fold Cross-Validation Results}
\end{table}
...
\bottomrule
\end{tabular}
\caption{SGD Classification Report}
\end{table}
...
\begin{itemize}
    ...
    \item Deny: AUC ≈ 1.00  
    \item Drop: AUC ≈ 1.00
    \item Reset-both: AUC ≈ 0.50 (random performance due to sample size)
\end{itemize}
...
\bottomrule
\end{tabular}
\caption{Computational Time Comparison (*estimated)}
\end{table}
